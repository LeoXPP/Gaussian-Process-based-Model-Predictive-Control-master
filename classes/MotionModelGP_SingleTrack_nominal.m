...
%--------------------------------------------------------------------------
    ...
    properties(Constant)
        M    = 500*0.95      % vehicle mass
        I_z  = 600*0.95      % vehicle moment of inertia (yaw axis)
        g    = 9.81     % gravitation
        ...
        c_f = 14000*1.2  % = 1*g*M/deltamax  % front coornering stiffness (C*delta=Fy~M*a)
        c_r = 14000*1.2  % = 2*g*M/deltamax  % rear coornering stiffness
    end
    ...
    methods
        ...
        %------------------------------------------------------------------
            ...
            if isa(x,'sym')
                syms M I_z l_f l_r deltamax maxbrakeWForce maxmotorWForce c_f c_r real
            else
                M = obj.M;
                I_z  = obj.I_z;
                l_f  = obj.l_f;
        ...
        %------------------------------------------------------------------
            ...
            u = [delta T track_vel]';
            syms M I_z l_f l_r deltamax maxbrakeWForce maxmotorWForce c_f c_r real
            params = [M I_z l_f l_r deltamax maxbrakeWForce maxmotorWForce c_f c_r]';
